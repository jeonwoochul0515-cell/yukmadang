      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline justify-between gap-3 flex-wrap">
          <div className="flex items-baseline gap-3">
            <h4 className="font-display text-lg sm:text-xl text-hanji">{item.name}</h4>
            {item.weight && (
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 relative">
        <div className="max-w-3xl mb-16">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-ember text-xs tracking-[0.3em] uppercase">Full Menu</span>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-x-16 gap-y-10">
          ...
          >
            <h3 className="font-display text-2xl text-ember-glow mb-2">
              구이 단품
            </h3>
            <p className="text-xs text-bone/50 mb-2">참숯에서 직원이 직접 구워드립니다.</p>
            <div>
              {cuts.map((item) => (
          ...
          >
            ...
            <p className="text-xs text-bone/50 mb-2">고기 다음, 마무리의 한 그릇.</p>
            <div>
              {sides.map((item) => (
                <Row key={item.id} item={item} />
              ))}
            ...
            <div className="mt-10 rounded-xl border border-charcoal-600/60 bg-charcoal-700/40 p-6">
              <p className="text-xs text-ember tracking-[0.2em] uppercase mb-3">Coupon</p>
              <p className="text-hanji font-medium mb-2">
                알림받기 고객에게 드리는 쿠폰
              </p>
              <p className="text-sm text-bone/70 leading-relaxed">
                가브리 1인분 또는 하이볼 1잔 — 네이버 플레이스에서 알림 받기 후 방문.
              </p>
            </div>
          </motion.div>
        </div>
